...
bool FileReader::readRules(std::string filename)
{
    std::ifstream file(filename);

    if (file.fail()) { throw std::string("File opening error!"); }
...
bool FileReader::isValid(std::string str)
{
    ...
    else
    {
        if (matchStr.ready())
        {
            ...
            std::string rightPart = matchStr.str(2);
            while (true) {
